<#
.DESCRIPTION
    Nexthink Remote Action script to install the Teams Meeting Add-in for Microsoft Office. This script downloads then uses the teamsbootstrapper.exe installer, and the --installTMA switch.
#>
...
# Execute teamsbootstrapper.exe with the -p switch for a per-machine install, and to ensure that the MSTeams installation is up to date, and the --installTMA switch to install the Teams Meeting Add-in for Microsoft Office
& "C:\Temp\teamsbootstrapper.exe" -p --installTMA | Out-Null
...
If ($LASTEXITCODE -eq 0) {
    ...
    $nxtSession['Message'] = "teamsbootstrapper.exe failed to install the Teams Meeting Add-in. Exit code: $LASTEXITCODE"
}
